describe('Ensure EAVs receive a notification when they make a change to their listing', function () {
    ...
    it('Trigger a notification to be sent to the test email inbox - half automated - half manual testing', function () {
        login.login_as(serviceConfig.EAVsOnly.NewPatel22, serviceConfig.password.pass);
        var dropdown = element(by.id('edit-centre'));
        dropdown.element(by.cssContainingText('option', 'NewPatel.22')).click();
        browser.sleep(10000)
